...
if __name__ == "__main__":
    ...
    for ind, node in enumerate(G_nodes):
        node_2_id[node] = ind
        id2node[ind] = node
    pickle.dump(id2node, open('data/ppi/ppi_id_2_node.p', 'wb'))
    pickle.dump(node_2_id, open('data/ppi/ppi_node_2_id.p', 'wb'))
    # --------------------------------------------
    ...
    pickle.dump(dsd_matrix, open('data/ppi/PPI_DSD_100.p', 'wb'))
    # ------------------- KL & JS ------------
    results_kl_js = kl_divergence_matrix(rw_matrix)
    kl_matrix = np.array(results_kl_js['RWKL'])
    js_matrix = np.array(results_kl_js['RWJS'])

    pickle.dump(kl_matrix, open('data/ppi/PPI_KL_100.p', 'wb'))
    pickle.dump(js_matrix, open('data/ppi/PPI_JS_100.p', 'wb'))